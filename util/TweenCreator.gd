...
static func cancel(tw: Tween):
	if tw and tw.is_running():
		tw.kill()

static func create(node: Node, tw: Tween):
	cancel(tw)
	return node.create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC).set_parallel()


func _init(n: Node, b: bool = true):
	node = n
	block = b
